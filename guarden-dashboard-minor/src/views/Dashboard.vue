<template>
  <main class="max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-8">
    ...
    <section class="grid grid-cols-3 gap-3 animate-fade-in" style="animation-delay:100ms">
      ...
      >
        <div class="text-[10px] font-mono text-garden-dim uppercase tracking-widest">{{ stat.label }}</div>
        <div class="font-mono font-bold text-xl" :style="{ color: stat.color }">{{ stat.value }}</div>
      </div>
    </section>
...
if (isDemoMode) {
  const updateDemo = () => {
    const loaded = zones.value.filter(z => !z.loading && !z.error)
    if (!loaded.length) return
    const avg = (key) => {
      const vals = loaded.map(z => z.sensors?.[key]).filter(v => v !== null && v !== undefined)
  ...
  const t = setInterval(updateDemo, 4000)
  unsubAvg = () => clearInterval(t)
  setTimeout(updateDemo, 500)
} else {
...
const summaryStats = computed(() => [
  { label: 'Avg. Soil Moisture', value: averages.value.moisture    !== null ? `${averages.value.moisture} %`    : '—', color: '#69F0AE' },
  { label: 'Avg. Temperature',   value: averages.value.temperature !== null ? `${averages.value.temperature} °C` : '—', color: '#FFD740' },
  { label: 'Avg. Humidity',      value: averages.value.humidity    !== null ? `${averages.value.humidity} %`    : '—', color: '#CE93D8' },
])
</script>
