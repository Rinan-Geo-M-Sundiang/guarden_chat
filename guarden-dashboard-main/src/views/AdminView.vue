<template>
  <main class="max-w-5xl mx-auto px-4 sm:px-6 py-8 space-y-8">
    ...
    <!-- Loading -->
    <div v-if="loading" class="flex items-center gap-3 text-garden-dim font-mono text-sm py-12 justify-center">
    ...
    <!-- User table -->
    <section v-else class="animate-slide-up rounded-2xl border border-garden-border overflow-hidden">
      <table class="w-full text-sm">
        <thead>
          <tr class="bg-garden-card border-b border-garden-border">
            <th class="text-left px-5 py-3 font-mono text-[10px] text-garden-dim uppercase tracking-widest">User</th>
            <th class="text-left px-5 py-3 font-mono text-[10px] text-garden-dim uppercase tracking-widest hidden md:table-cell">Last Login</th>
            <th class="text-left px-5 py-3 font-mono text-[10px] text-garden-dim uppercase tracking-widest">Role</th>
            <th class="text-right px-5 py-3 font-mono text-[10px] text-garden-dim uppercase tracking-widest">Actions</th>
          </tr>
        </thead>
        <tbody>
          <tr
          ...
          >
              ...
              <div class="flex items-center gap-3">
                ...
                <div>
                  <div class="font-sans font-medium text-garden-text text-sm">
                    {{ user.displayName || '—' }}
                    <span v-if="user.uid === currentUser?.uid"
            ...
            <td class="px-5 py-4">
              <span
                class="inline-flex px-2.5 py-1 rounded-full font-mono text-[10px] uppercase tracking-widest font-medium"
            ...
            <td class="px-5 py-4 text-right">
              <div class="flex items-center justify-end gap-2">
                ...
                <button
                  v-else
                  class="font-mono text-[11px] px-3 py-1.5 rounded-lg border
                         border-garden-border text-garden-dim
                         hover:bg-garden-card transition-colors
                         disabled:opacity-40 disabled:cursor-not-allowed"
                  :disabled="actionTarget === user.uid || user.uid === currentUser?.uid"
      ...
      >
        <div class="bg-garden-surface border border-garden-border rounded-2xl
          ...
          <p class="text-sm text-garden-dim">
            This will remove
            <strong class="text-garden-text">{{ deleteTarget.displayName || deleteTarget.email }}</strong>
...
const actionTarget = ref(null)  // uid of user currently being acted on
const deleteTarget = ref(null)  // user object pending delete confirmation
  ...
  onValue(
    ...
    (snapshot) => {
      ...
      users.value = Object.entries(raw)
        ...
        // Filter out "device" role accounts and accounts with no role
...
async function removeUser(user) {
  ...
  actionTarget.value = user.uid
  try {
    // Remove the user node from RTDB.
    // Note: this does NOT delete their Firebase Auth account.
    // For full deletion use Firebase Admin SDK server-side.
