      ...
      <div v-if="showSettings" class="modal-overlay" @click="closeSettings">
        <div class="modal-content" @click.stop>
          ...
          <div class="modal-body">
            ...
            <!-- changed: new section for moisture relay thresholds -->
            <div class="settings-section">
...
// changed: added moistureOn and moistureOff to default thresholds
...
async function loadThresholds() {
  ...
  try {
    ...
    if (snapshot.exists()) {
      thresholds.value = { ...thresholds.value, ...snapshot.val() } // changed: spread preserves moistureOn/Off defaults if not yet in Firebase
...
async function saveThresholds() {
  ...
  try {
    loading.value = true
    await set(dbRef(db, 'config/thresholds'), editThresholds.value) // changed: editThresholds now includes moistureOn/Off so they are written to Firebase
